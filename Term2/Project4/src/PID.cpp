...
void PID::Init(double kp, double ki, double kd) {
    ...
    _dK[PIDEntry::D] = kd > 0.000005 ? kd * 0.05 : 1;
    _dK[PIDEntry::P] = kp > 0.000005 ? kp * 0.05 : 1;
    _dK[PIDEntry::I] = ki > 0.000005 ? ki * 0.05 : 1;
    _iterations = 0;
    _best_error = std::numeric_limits<double>::max();
    _cumulative_cuadratic_error = 0;
    _current_twiddle_state = TwiddleState::Start;
    _current_twiddle_variable = PIDEntry::I;
}

void PID::UpdateError(double cte) {
    _error[PIDEntry::D] = _error[PIDEntry::P] - cte;
