...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    auto possible_next_states = successor_states();
    map<string, float> costs;
    map<string, vector<Vehicle>> trajectories;
    for (auto s = possible_next_states.begin(); s != possible_next_states.end(); s++)
    {
        vector<Vehicle> trajectory = generate_trajectory(*s, predictions);
        trajectories.insert({*s, trajectory});
        float trajectory_cost = calculate_cost(*this, predictions, trajectory);
        costs.insert({*s, trajectory_cost});
    }
    string newState = possible_next_states[0];
    for (auto s = possible_next_states.begin(); s != possible_next_states.end(); s++)
    {
